...
public class FilteredSQUIDController {
     ...
     */
    public double runSQUID() {

        double proportionalTerm = Math.signum(error) * Math.sqrt(Math.abs(error)) * P();
        double derivativeTerm = Math.signum(filteredDerivative) * Math.sqrt(Math.abs(filteredDerivative)) * D();
        return proportionalTerm + derivativeTerm + errorIntegral * I() + F();
    }

    /**
     * This can be used to update the filtered SQUID's current position when inputting a current position and
